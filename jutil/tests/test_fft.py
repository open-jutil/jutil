...
def test_fft():
    for n in NS:
        ...
        assert_almost_equal(fftmod.fft(x), jfft._fft(x))


def test_ifft():
    for n in NS:
        x = np.random.random(n) + 1j * np.random.random(n)
        assert_almost_equal(fftmod.ifft(x), jfft._ifft(x))


def test_rfft():
    for n in NS:
        x = np.random.random(n)
        assert_almost_equal(fftmod.rfft(x), jfft._rfft(x))
...
def test_irfft():
    for n in NS:
        ...
        assert_almost_equal(fftmod.irfft(x, n=n), jfft._irfft(x, n=n))
...
def test_rfft2():
    for n1 in NS:
        for n2 in NS:
            ...
            assert_almost_equal(fftmod.rfft2(x), jfft._rfft2(x))
...
def test_irfft2():
    for n1 in NS:
        for n2 in NS:
            ...
            assert_almost_equal(fftmod.irfft2(x, s=(n1, n2)), jfft._irfft2(x, n=n2))


def test_fft_adj():
    for n in NS:
        A = jdiff.fd_jac(wrap(fftmod.fft, None, True, True), np.zeros(2 * n))
        B = jdiff.fd_jac(wrap(jfft.fft_adj, None, True, True), np.zeros(2 * n)).T
        print  A - B
        assert_almost_equal(abs(A - B).sum(), 0, err_msg=str((A, B)), decimal=6)
        ...
        print ATx_1 - ATx_2
        print "oooO"
        assert_almost_equal(ATx_1, ATx_2, decimal=6)


def test_ifft_adj():
    for n in NS:
        A = jdiff.fd_jac(wrap(fftmod.ifft, None, True, True), np.zeros(2 * n))
...
def test_rfft_adj():
    for n in NS:
        ...
        A = jdiff.fd_jac(wrap(fftmod.rfft, None, False, True), np.zeros(n))
...
def test_irfft_adj():
    for n in NS:
        ...
        A = jdiff.fd_jac(wrap(fftmod.irfft, m, True, False, {"n":n}), np.zeros(m * 2))
...
def test_rfft2_adj():
    for n1 in NS:
        for n2 in NS:
            ...
            A = jdiff.fd_jac(wrap(fftmod.rfft2, (n1, n2), False, True), np.zeros(n1 * n2))
...
def test_irfft2_adj():
    for n1 in [2, 4, 10, 11, 16, 17, 21]:
        for n2 in [2, 4, 10, 11, 16, 17, 21]:
            ...
            A = jdiff.fd_jac(wrap(fftmod.irfft2, (n1, m), True, False, {"s":(n1, n2)}),
                             np.zeros(n1 * m * 2))
